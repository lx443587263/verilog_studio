...
void VerilogStudio::CmdParse::CmdLineParse(int argc, char **argv) {
    ...
    if (cmdparse->exist("input")) {
        ...
        //tr = pHierarchy->CreateTree(TopModuleName, pParse);
    }
...
/**********************************************/
void VerilogStudio::CmdParse::AddGroup(string &connectRule) {
    auto tr = pHierarchy->CreateTree(TopModuleName, pParse);
    htree<string>::iterator iter(tr.root);
    vector<string> InstModuleNamePath,leftPortNameVec,rightPortNameVec,leftPortTypeVec,PortName,rightPortTypeVec;
    string leftPortName, rightPortName, virTopModule,leftGroupModuleName,rightGroupModuleName,tempInstGroupPortName;
    if (connectRule.find("=>") != string::npos) {
        ToTopFlag = false;
        vector<string> tempPath = split(connectRule, "=>");
        //tempPath[1] = Trim(tempPath[1]);
        ...
        path1.pop_back();
        path2.pop_back();
        leftGroupModuleName = path1.back();
        rightGroupModuleName = path2.back();
        if (tr.root->ModuleName.empty()) {
            logger->error("tree is empty");
            cout << "tree is empty()" << endl;
        }
        ...
        string leftGroupSourceName = pParse->GetSourceFileName(leftGroupModuleName);
        string rightGroupSourceName  = pParse->GetSourceFileName(rightGroupModuleName);
        string templeftPortName = pChangeLine->GetGroupPortName(leftGroupSourceName,leftPortName);
        string temprightPortName = pChangeLine->GetGroupPortName(rightGroupSourceName,rightPortName);
        leftPortTypeVec = split(templeftPortName,"\n");
        rightPortTypeVec = split(temprightPortName,"\n");
        if(rightPortTypeVec.empty())
        {
            cout << "rightPortTypeVec is empty"<<endl;
        }
        leftPortNameVec = pChangeLine->GetPortName(leftPortTypeVec);
        rightPortNameVec = pChangeLine->GetPortName(rightPortTypeVec);
        //PortName = templeftPortName;
        ...
        cout << "rightPortName:" << rightPortName << endl;
        //cout << "portName:" << PortName << endl;
        cout << "leftGroupSourceName" << leftGroupSourceName <<endl;
        //cout << "rightGroupSourceName" << rightGroupSourceName <<endl;
        logger->info("tempath[0]:{}", tempPath[0]);
        logger->info("tempath[1]:{}", tempPath[1]);
        logger->info("leftPortName:{}", leftPortName);
        logger->info("rightPortName:{}", rightPortName);
...
//            cout << "tree is empty" << endl;
        ...
        string leftGroupSourceName = pParse->GetSourceFileName(leftGroupModuleName);
        string templeftPortName = pChangeLine->GetGroupPortName(leftGroupSourceName,leftPortName);
        leftPortNameVec = pChangeLine->GetPortName(split(templeftPortName,"\n"));
        leftPortTypeVec = split(templeftPortName,"\n");
        //PortName = templeftPortName;
    ...
    string getVirTopModuleFile = pParse->GetSourceFileName(virTopModule);
    pChangeLine->GetTopModuleName(getVirTopModuleFile);
    auto findResultIndex = find(InstModuleNamePath.begin(),InstModuleNamePath.end(),FlipModule);
    int FlipModuleIndex = distance(InstModuleNamePath.begin(),findResultIndex);
    for (auto &it: InstModuleNamePath) {
        string tempSourceFileName = pParse->GetSourceFileName(it);
        string tempPortEnd = pParse->GetPortEnd(it);
        string tempSrcModule = pParse->GetSourceModuleName(it);
        ...
        int tempEndBracketsLocation = pParse->GetEndBracketsLocation(it);
        string tempInstFileName = pParse->GetInstLocationFileName(it);
        string tempInstGroupPortName;
        auto findRes = find(InstModuleNamePath.begin(),InstModuleNamePath.end(),it);
        int CurrentIndex = distance(InstModuleNamePath.begin(),findRes);
        //vector<string> tempPortGather;
        if (ToTopFlag) {
            if(it!=leftGroupModuleName && it!=rightGroupModuleName) {
                PortName = pChangeLine->ComposeGroupPortName(leftPortTypeVec, leftPortNameVec,tempPortEnd);
                pChangeLine->AddGroupPort(tempSourceFileName,PortName,tempPortEnd, tempSrcModule, tempBracketsLocation, tempEndBracketsLocation);
            }
            if (it != virTopModule) {
                pChangeLine->AddInstPort(tempInstFileName, tempInstGroupPortName, it);
            }
        } else {
            if (it == virTopModule) {
                pChangeLine->AddGroupToTopModule(tempSourceFileName, leftPortNameVec, tempPortEnd, tempBracketsLocation, tempEndBracketsLocation);
            } else {
                tempInstGroupPortName = pChangeLine->InstGroupPortName(tempInstFileName,leftPortNameVec);
                PortName = pChangeLine->ComposeGroupPortName(leftPortTypeVec,leftPortNameVec,tempPortEnd);
                if (CurrentIndex >= FlipModuleIndex) {
                    if (rightPortName == "No") {
                        //PortName = pChangeLine->FlipPort(leftPortName);
                    } else {
                        tempInstGroupPortName = pChangeLine->InstGroupPortName(tempInstFileName,rightPortNameVec);
                        PortName = pChangeLine->ComposeGroupPortName(rightPortTypeVec,rightPortNameVec,tempPortEnd);
                    }
                }
                if(it!=leftGroupModuleName && it!=rightGroupModuleName){
                    pChangeLine->AddGroupPort(tempSourceFileName, PortName, tempPortEnd, tempSrcModule, tempBracketsLocation, tempEndBracketsLocation);
